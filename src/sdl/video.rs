...
pub fn is_video_mode_ok(w: isize, h: isize, bpp: isize,
	...
	unsafe {
		...
		if bpp == 0 { None } else { Some(bpp as isize) }
	}
}
...
fn wrap_video_info_flags(bitflags: u32) -> Vec<VideoInfoFlag> {
	...
	flags.iter().filter_map(|&flag| {
		if bitflags & (flag as u32) != 0 { Some(flag) } else { None }
	}).collect()
}
...
impl Surface {
	...
	pub fn from_bmp(path: impl Into<String>) -> Result<Surface, String> {
		...
		if raw.is_null() { Err(get_error()) } else { Ok(wrap_surface(raw, true)) }
	}
	...
	pub fn set_colors(&self, colors: &[Color]) -> bool {
		...
		unsafe {
			ll::SDL_SetColors(self.raw, colors.as_mut_ptr(), 0,
	...
	pub fn set_palette(&self, palettes: &[PaletteType],
		...
		unsafe {
			ll::SDL_SetPalette(self.raw, flags,
			                   colors.as_mut_ptr(), 0,
			                   colors.len() as c_int) == 1
		}
	}
	...
	pub fn fill_rect(&self, rect: Option<Rect>,
	                 color: Color) -> bool {
		unsafe {
			ll::SDL_FillRect(self.raw, match rect {
				Some(ref rect) => mem::transmute(rect),
				None => ptr::null_mut()
			}, color.to_mapped((*self.raw).format as *const _)) == 0
		}
...
pub fn set_gamma(r: f32, g: f32, b: f32) -> bool {
	unsafe {
		ll::SDL_SetGamma(r as c_float, g as c_float,
		                 b as c_float) != -1
	}
}
...
pub fn get_gamma_ramp() -> ([u16; 256], [u16; 256], [u16; 256]) {
	...
	unsafe {
		ll::SDL_GetGammaRamp(r.as_mut_ptr(),
		                     g.as_mut_ptr(),
		                     b.as_mut_ptr());
	}
